            ...
            <div class="col-span-6 lg:col-span-3 flex justify-between items-center w-full flex-col sm:flex-row">
                <div class="flex my-5 sm:my-0 text-gray-300 font-bold text-[10px] sm:text-xs">
                    <div class="flex flex-col h-full ml-1 bg-gray-100 bg-opacity-60 w-[55px] h-[50px] items-center justify-center rounded-[100%]">
                        <img src="https://api.cybersportscore.com/media/event/_120/e{{ $match->getTournament()->id }}.webp" title="{{ $match->getTournament()->title }}"
                             alt="{{ $match->getTournament()->title }} icon" loading="lazy" class="w-[2.5rem] h-[2.5rem] inline">
                    </div>
                ...
                <div class="flex flex-row align-center md:w-56 md:justify-center sm:justify-end sm:ml-3 flex-wrap">
                    @for ($numMap = 1; $numMap <= $match->getBestOf(); $numMap++)
                        <span class="mx-1 sm:mx-2 my-1 sm:mr-0">
                            <a aria-current="page" href="/522461?num={{$numMap}}" class="
                                ...
                                uppercase text-[10px] font-semibold px-2 py-1 rounded sm:text-xs">
                                    {!! $match->getNum() == $match->getLiveNum() && !$match->getWinner() ? "<span class='animate-pulse inline-flex w-[8px] h-[8px] bg-red-500 border border-gray-400 border-1 rounded-[100%] mr-1'></span>" : "" !!}
                                <span class="inline-flex" >
                                    <span data-translate="labels.map">Карта</span>
                                    &nbsp;
                                    <span> {{ $numMap }}</span>
                                </span>
                            </a>
