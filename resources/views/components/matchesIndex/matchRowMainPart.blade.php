...
<div class="items-end flex-col items-col grow w-8 sm:w-10 overflow-visible text-xs text-gray-300 md:text-base" >
    <div id="t1-info" class="flex-col-reverse items-end flex" >
        ...
        <span class="text-[10px] sm:text-xs font-semibold mr-3 sm:mr-0 mb-0 sm:mb-1 md:mb-0 truncate" >
            {{ $t1 ? $preview->getTeam1()->getShortTitle() : $preview->getTeam2()->getShortTitle() }}
        </span>
    </div>
</div>
<div id="t1-logo" class="items-col w-[50px] md:w-[60px] items-center hidden sm:flex" >
    <img src="https://api.cybersportscore.com/media/logo/_30/t{{$t1 ? $preview->getTeam1()->id : $preview->getTeam2()->id }}.webp" alt="MIRACLE ESPORT icon" class="w-[1.6rem] aspect-[3/2] object-contain" loading="lazy" >
</div>
    ...
    <div >
        ...
        <div id="score-match" class="flex flex-row items-center text-base justify-center italic" >
            <span class="leading-normal text-apple text-xs sm:text-sm text-right font-bold pr-1">
                {{ $t1 ? $match_beta->match_games[$num_game - 1]->match_data->teams->t2->score
            ...
            {{ $t2 ? $match_beta->match_games[$num_game - 1]->match_data->teams->t1->score
...
<div id="t2-logo" class="items-col w-[50px] md:w-[60px] items-center hidden sm:flex">
    <img src="https://api.cybersportscore.com/media/logo/_30/t{{$t2 ? $preview->getTeam2()->id : $preview->getTeam1()->id }}.webp" alt="Vision icon" class="w-[1.6rem] aspect-[3/2] object-contain" loading="lazy" >
</div>
<div class="items-col grow w-8 sm:w-10 overflow-visible text-xs text-gray-300 md:text-base" >
    <div id="t2-info" class="flex-col flex" >
        <span class="text-[10px] sm:text-xs font-semibold ml-3 sm:ml-0 mb-0 sm:mb-1 md:mb-0 truncate" >
            {{$t2 ? $preview->getTeam2()->getShortTitle() : $preview->getTeam1()->getShortTitle()}}
        </span>
...
<div class="items-col w-[50px] items-center">
    ...
    <span id="score-games" class="bg-apple text-[11px] leading-none font-semibold mt-0.5 w-7 px-1 py-0.5 rounded text-white text-center">
        {{ $t1 ? $preview->getTeam1()->getMatchScore() : $preview->getTeam2()->getMatchScore() }}:{{ $t2 ? $preview->getTeam2()->getMatchScore() : $preview->getTeam1()->getMatchScore()}}
    </span>
</div>
