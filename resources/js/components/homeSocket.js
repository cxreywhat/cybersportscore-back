...
function scoreMap(duration, scoreT1, scoreT2, timeStartMatchSeconds, isLive, bbOdds) {
    ...
    infoScore.className = 'border-l border-r text-gray-400 items-center justify-content sm:text-sm w-[70px] sm:w-[100px]';

    infoScore.style.display = "flex";
    infoScore.style.height = "100%";
    infoScore.style.flexDirection = "column";
    infoScore.style.justifyContent = "center";
    infoScore.style.borderColor = "rgb(55 65 81 / var(--tw-border-opacity))";
    infoScore.style.color = "#718096";
...
function teamTitle(shortTitleTeam) {
    return `
        <div class="items-end flex-col grow w-8 sm:w-10 overflow-visible text-xs  md:text-base text-end" style="
            display: flex;
            height: 100%;
...
function teamAdvAndTitle(shortTitleTeam, sideTeam) {
    ...
    container.className = (sideTeam === 'team-left' ? 'items-end' : 'items-start') + " flex-col grow w-8 sm:w-10 overflow-visible text-xs text-gray-300 md:text-base";
    container.style.display = 'flex' ;
    container.style.height = '100%';
    container.style.flexDirection = 'column';
    container.style.justifyContent = 'center';
    container.style. borderColor = 'rgb(55 65 81 / var(--tw-border-opacity))';
...
function scoreMatch(seriesBO, t1ScoreMatch, t2ScoreMatch, matchDate, isLive) {
    return `
    <div class="w-[50px] items-center" style="
        display: flex;
        height: 100%;
        flex-direction: column;
        justify-content: center;
        ...
        color: #718096;">
            ...
            <span class="${matchDate < Math.round(Date.now() / 1000) && isLive ? 'bg-apple' : 'bg-gray-500'} text-[11px] leading-none font-semibold mt-0.5 w-7 px-1 py-0.5 rounded text-white mx-auto text-center">
                ${t1ScoreMatch}:${t2ScoreMatch}
            </span>
        </div>`
}
...
function logoTeam(idTeam) {
    return `
        <div class="w-[50px] md:w-[60px] items-center hidden sm:flex" style="
            display: flex;
            height: 100%;
            flex-direction: column;
            justify-content: center;
            ...
            <div class="flex flex-col items-center mx-auto">
                <img src="/media/logo/_30/t${idTeam}.webp"
                     alt="team icon" class="w-[1.6rem] aspect-[3/2] object-contain" loading="lazy">
            </div>
        </div>
        `
}
...
function dateMatch(isLive, numMap, matchDate, matchId) {
    return `
        <div class="w-[45px] sm:w-[120px] md:w-[130px] p-0 sm:px-3 py-3 items-start text-sm" style="
            display: flex;
            height: 100%;
            flex-direction: column;
            justify-content: center;
            ...
            ${matchDate < Math.round(Date.now() / 1000) && isLive? `
                ...
                `<div class="w-[45px] sm:w-[120px] md:w-[130px]  py-3 leading-[1rem] text-[9px]" style="
                    display: flex;
                    height: 100%;
                    flex-direction: column;
                    justify-content: center;
...
function eventLogo(eventId) {
    ...
    return `
    <div class="w-[80px] hidden sm:flex items-center" style="
        display: flex;
        height: 100%;
        flex-direction: column;
        justify-content: center;
...
export function createMatch(match) {
    ...
    matchDiv.style.backgroundColor = 'rgba(55, 65, 81, 0.2)';
    matchDiv.style.color = '#718096';
    matchDiv.style.alignItems = 'center';
    matchDiv.style.willChange = 'contents';

    matchDiv.setAttribute("data-game", `${match.game_id === 582 ? '582' : '313' }`);
    matchDiv.setAttribute("data-tournament", `${ match.tournament_id }`);
