...
export function generatePlayerDetails(player, player2, bigNet, gameId) {
    ...
    details.innerHTML = `
        ...
        <div class='justify-end px-2 flex w-full items-center grow max-w-[50%] relative max-h-8'>
            <span title='Ценность героя ${player2.net_worth}' class='right-2 origin-right hover:bg-clip-border py-[2px] -bottom-[9px] opacity-90 bg-yellow-400 transition
              font-bold absolute w-full border border-t-[1px] border-b-[1px] border-transparent bg-clip-padding' style='transform: scaleX(${player2.net_worth * 0.9 / bigNet})'>
            </span>
            <span title='${player2.level} Level' class='mr-2 font-semibold bg-gray-700 bg-opacity-20 px-1 py-0
              leading-normal cursor-default text-[8px] text-gray-500 border-2 border-gray-700 hover:border-gray-600 rounded'>${player2.level}</span>
